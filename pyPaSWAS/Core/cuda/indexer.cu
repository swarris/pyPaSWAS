...
__global__ void calculateDistance(int *index, int *query, float *distances, float scale){
	...
	unsigned int seq = threadIdx.y; 
	s_distances[thread] = (float) index[block+threadPlus1] - (float)query[threadPlus1+seq];
	...
	if (thread == 0){
		distances[blockIdx.x*BLOCK_SIZE+blockIdx.y+seq] =  sqrt(s_distances[INDEX_SIZE-1])/scale;
	}
}
